...
5. **Notify pa-infra-manager** that the IaC changes are completed. Do NOT commit or stage any files; all commits are handled by the common `pa-committer` at the end of the development phase.
6. **Escalate ambiguity** — if the Implementation Plan is unclear or conflicts with existing code in a way you cannot resolve, escalate to `pa-infra-manager` with the specific question. Do not guess or silently deviate from the plan.
